// ä¸´æåç¥ â IndexedDB æ°æ®å±
// ä½¿ç¨å¨å± idbï¼UMDï¼åº
(function() {
  'use strict';
  ...
  async function updateFood(id, patch) {
    ...
    if (!existing) throw new Error('é£åä¸å­å¨');
  ...
  async function importAll(data, merge = false) {
    if (!data || !data.foods) throw new Error('æ°æ®æ ¼å¼éè¯¯');
    const db = await getDB();
